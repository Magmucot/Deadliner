...
new_items = []
new_strings = [
    ("Overview of upcoming reminders, active break profiles and today's events.", "Обзор предстоящих напоминаний, активных профилей и событий."),
    ("Manage one-time and recurring reminders.", "Управление разовыми и регулярными напоминаниями."),
    ("Tune reminder modes, break duration and snooze rules.", "Настройка режимов напоминаний, перерывов и правил откладывания."),
    ...
    ("Paused", "На паузе"),
    ("Events", "События"),
    ("One-time and recurring reminders with profile-based behavior.", "Разовые и повторяющиеся напоминания с привязкой к профилям."),
    ("Next trigger", "Следующее"),
    ("State", "Статус"),
    ("Profiles", "Профили"),
    ("Profiles define reminder intensity, snooze limits and break behavior.", "Профили задают интенсивность, лимиты откладывания и перерывы."),
    ("Create at least one profile to control reminder behavior.", "Создайте хотя бы один профиль для управления напоминаниями."),
    ("Name", "Название"),
    ("Scope", "Назначение"),
    ("Origin", "Источник"),
    ("%1 / %2 min", "%1 / %2 мин"),
    ...
    ("Snoozed", "Отложено"),
    ("Date", "Дата"),
    ("Lightweight local analytics for completed, skipped and snoozed reminders.", "Локальная статистика по выполненным и пропущенным напоминаниям."),
    ...
    ("Control startup behavior, quiet hours, theme and interface language.", "Управление запуском, режимом тишины, темой и языком интерфейса."),
    ...
    ("Tray integration is not available right now, so the main window stays as the primary control surface.", "Трей сейчас недоступен, поэтому главное окно остаётся открытым."),
    ...
    ("The application supports Russian and English out of the box, and you can switch later in Settings without restarting.", "Приложение поддерживает русский и английский, язык всегда можно сменить в настройках."),
    ("Break reminders that respect your day", "Напоминания, которые уважают ваше время"),
    ("Deadliner starts gently, escalates only when needed, and keeps quiet hours configurable so reminders stay useful instead of annoying.", "Deadliner начинает мягко и использует тихие часы, чтобы напоминания были полезными, а не раздражающими."),
    ("Desktop-first workflow", "Удобство на десктопе"),
    ("Most daily actions are available from the system tray, while the main window keeps event, profile and statistics management organized.", "Частые действия доступны из трея, а главное окно управляет событиями и статистикой."),
    ("Recommended starter setup", "Рекомендуемый пресет"),
    ("You can start with a safe eye-rest preset now and fine-tune it later in Profiles.", "Вы можете начать с базового правила отдыха для глаз и настроить его позже."),
    ("Enable the recommended 20-20-20 inspired eye-rest rule (20 min work / 1 min break).", "Включить рекомендуемое правило 20-20-20 (20 мин работы / 1 мин отдыха глаз)."),
    ("Back", "Назад"),
    ("Next", "Далее"),
    ("Finish", "Завершить"),
    ("Events inherit reminder mode and snooze policy from the selected profile.", "События наследуют режим напоминания и лимиты от профиля."),
