...
QDateTime nextDateForWeekdays(const QDateTime &start, const QDateTime &reference, const QList<int> &days)
{
    QDateTime candidate(reference.date(), start.time(), reference.timeZone());
    if (candidate < reference) {
...
QDateTime computeNextDateTimeOccurrence(const ReminderEvent &event, const QDateTime &reference)
{
    ...
    if (event.recurrenceRule == QStringLiteral("daily")) {
        QDateTime candidate(reference.date(), event.startAt.time(), reference.timeZone());
        if (candidate < reference) {
            candidate = candidate.addDays(1);
        }
        return candidate;
    ...
    if (event.recurrenceRule == QStringLiteral("weekdays")) {
        return nextDateForWeekdays(event.startAt, reference, {1, 2, 3, 4, 5});
    }

    if (event.recurrenceRule.startsWith(QStringLiteral("weekly:"))) {
        return nextDateForWeekdays(event.startAt, reference, parseWeekdays(event.recurrenceRule));
    }
    ...
    if (event.recurrenceRule.startsWith(QStringLiteral("monthly:"))) {
        ...
        const QDateTime normalized(reference.date(), event.startAt.time(), reference.timeZone());
