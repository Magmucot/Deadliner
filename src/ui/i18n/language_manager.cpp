...
namespace deadliner::ui
{
    ...
    namespace
    {
        ...
        const QHash<QString, QString> HybridTranslator::s_fallback = {
            ...
            {QStringLiteral("*\n20-20-20 eye rest"), QStringLiteral("Отдых глаз 20-20-20")},
            {QStringLiteral("*\nShort eye-rest reminder suggested during onboarding."),
             QStringLiteral("Короткое напоминание для отдыха глаз, предложенное при первичной настройке.")},
            {QStringLiteral("*\nNo quiet hours"), QStringLiteral("Без тихих часов")},
            {QStringLiteral("*\nMode: %1\nSnooze: %2 x %3 min\nBreak: %4 min"),
            ...
            {QStringLiteral("*\nProfiles define reminder intensity, snooze limits and break behavior."),
             QStringLiteral("Профили определяют интенсивность напоминаний, лимиты откладывания и режим перерывов.")},
            {QStringLiteral("*\nLightweight local analytics for completed, skipped and snoozed reminders."),
             QStringLiteral("Статистика по выполненным, пропущенным и отложенным напоминаниям.")},
            {QStringLiteral("*\nOne-time and recurring reminders with profile-based behavior."),
             QStringLiteral("Одноразовые и повторяющиеся напоминания с поведением на основе профиля.")},
            {QStringLiteral("*\nControl startup behavior, quiet hours, theme and interface language."),
             QStringLiteral("Настройки запуска, тихих часов, темы и языка интерфейса.")},
            // MainWindow subtitleForSection
            {QStringLiteral("*\nOverview of upcoming reminders, active break profiles and today's events."),
             QStringLiteral("Обзор предстоящих напоминаний, активных профилей перерывов и сегодняшних событий.")},
            {QStringLiteral("*\nManage one-time and recurring reminders."),
             QStringLiteral("Управление одноразовыми и повторяющимися напоминаниями.")},
            {QStringLiteral("*\nReview one-time reminders that are already past due."),
             QStringLiteral("Просмотр одноразовых напоминаний, которые уже просрочены.")},
            ...
            {QStringLiteral("*\nCheck local reminder outcomes and adherence trends."),
             QStringLiteral("Статистика выполнения напоминаний и тенденции.")},
            {QStringLiteral("*\nAdjust startup behavior, quiet hours, theme and language."),
             QStringLiteral("Настройки запуска, тихих часов, темы и языка.")},
        };
